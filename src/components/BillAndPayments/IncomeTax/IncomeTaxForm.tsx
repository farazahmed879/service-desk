...
import IncomeTaxList from "@/components/BillAndPayments/IncomeTax/IncomeTaxList";
...
export default function IncomeTaxForm({ serviceType }: ServiceFormProps) {
        ...
        <div className="w-full rounded-xl border border-gray-200 bg-white p-6 shadow-md">
            {!showForm ? (
                <>
                    ...
                    <IncomeTaxList records={records} onDelete={handleDelete} />
                </>
            ) : (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    <div className="flex items-center justify-between mb-4 border-b pb-4">
                        <h2 className="text-2xl font-bold text-gray-700 capitalize">Deposit Income Tax</h2>
                        <button
                            type="button"
                    ...
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <InputField label="NTN/CNIC" name="InvoiceNumber" register={register} placeholder="Enter NTN or CNIC" />
                        <InputField label="Payment Date" name="paymentDate" register={register} type="date" />
                        <InputField label="Amount" name="amount" register={register} placeholder="Enter Amount" type="number" />
                        <InputField label="Paid By" name="paidBy" register={register} placeholder="Enter Payer Name" />
                    </div>
                    <InputField label="Remarks" name="remarks" register={register} placeholder="Enter any remarks" textarea rows={3} />
