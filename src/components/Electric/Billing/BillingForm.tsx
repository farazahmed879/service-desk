...
interface ElectricBillingFormData {
    userName: string;
    billMonth: string;
    amount: string;
    dueDate: string;
...
export default function BillingForm({ serviceType }: ServiceFormProps) {
    const [showForm, setShowForm] = useState(false);
    const [isMounted, setIsMounted] = useState(false);
    const { register, handleSubmit, reset } = useForm<ElectricBillingFormData>({
        defaultValues: {
            userName: "",
            billMonth: "",
            amount: "",
    ...
    const onSubmit = (data: ElectricBillingFormData) => {
        ...
        <div className="w-full rounded-xl border border-gray-200 bg-white p-6 shadow-md">
            {!showForm ? (
                <>
                    ...
                    <BillingList records={records} onDelete={handleDelete} />
                </>
            ) : (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    <div className="flex items-center justify-between mb-4 border-b pb-4">
                        <h2 className="text-2xl font-bold text-gray-700 capitalize">Add New Electricity Bill Record</h2>
                        <button
                            type="button"
                    ...
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        ...
                        <InputField label="Billing Month" name="billMonth" register={register} placeholder="e.g. October 2023" />
                        <InputField label="Amount (Rs.)" name="amount" register={register} type="number" placeholder="Enter Bill Amount" />
                        <InputField label="Due Date" name="dueDate" register={register} type="date" />
                    </div>
