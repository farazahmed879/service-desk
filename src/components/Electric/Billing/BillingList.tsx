...
interface ElectricBillingRecord {
    id: string;
    userName: string;
    billMonth: string;
    amount: string;
...
interface ListProps {
    records: ElectricBillingRecord[];
    onDelete: (id: string) => void;
}

export default function BillingList({ records, onDelete }: ListProps) {
    const columns: Column<ElectricBillingRecord>[] = [
        {
            header: "User Name",
    ...
    return (
        <div className="w-full overflow-hidden">
            <DataTable
                data={records}
                columns={columns}
                emptyMessage="No electricity billing records found."
            />
        </div>
    );
}
