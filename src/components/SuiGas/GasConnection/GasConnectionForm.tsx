...
export default function GasConnectionForm({ serviceType }: ServiceFormProps) {
    const [showForm, setShowForm] = useState(false);
    const [isMounted, setIsMounted] = useState(false);
    const { register, handleSubmit, reset } = useForm<GasConnectionFormData>({
        defaultValues: {
            userName: "",
            consumerNumber: "",
    ...
    const onSubmit = (data: GasConnectionFormData) => {
        ...
        <div className="w-full rounded-xl border border-gray-200 bg-white p-6 shadow-md">
            {!showForm ? (
                <>
                    ...
                    <GasConnectionList records={records} onDelete={handleDelete} />
                </>
            ) : (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                    <div className="flex items-center justify-between mb-4 border-b pb-4">
                        <h2 className="text-2xl font-bold text-gray-700 capitalize">Apply for Gas Connection</h2>
                        <button
                            type="button"
